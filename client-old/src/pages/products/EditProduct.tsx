...
import { getProductById, deleteProductById, updateProductById } from '../../features/products/redux/productSlice';
...
export const EditProduct = () => {
	...
	const onSubmitEditProduct = async (data: IEditProductInputs) => {
		try {
			await dispatch(updateProductById({ idProduct: +id!, formValues: data }));
		} catch (error) {
			console.log(error);
		}
	};
		...
		<div className="mx-auto max-w-[90%] mt-[35px]">
			...
			<div className="gap-y-5 mb-[30px]">
				<div className="p-5 bg-blue-500 rounded-lg shadow-lg w-full">
					<UpdateForm idProduct={+id!} oneProduct={oneProduct} onSubmit={onSubmitEditProduct} isLoading={isLoading} />
				</div>
