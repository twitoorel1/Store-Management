...
const UpdateForm: FC<any> = ({ idProduct, oneProduct, onSubmit, isLoading }) => {
	const dispatch = useAppDispatch();
	// const { isLoading } = useAppSelector((state: RootState) => state.product);
	...
	const onSubmitEditProduct = async (data: IEditProductInputs) => {
		try {
			await onSubmit(data); // Use the provided onSubmit function
		} catch (error) {
			console.log(error);
		}
	};
